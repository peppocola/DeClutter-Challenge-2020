...
def plot_length():
    ...
    plt.legend(['non non-information', 'non-information'])
    plt.text(1000, 80, 'ni avg length= ' + str(round(sum(sample_ni) / len(sample_ni), 2)))
    plt.text(1000, 90, 'nni avg length=' + str(round(sum(sample_nni) / len(sample_nni), 2)))

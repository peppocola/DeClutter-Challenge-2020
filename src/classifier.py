...
def final_classify(stemming=True, rem_kws=True):
    ...
    features_test = get_features(set='test', stemming=stemming, rem_kws=rem_kws)
...
def classify_split(folder="split_classifier"):
    ...
    data_split()
    # TF-IDF
    comments_train = get_comments(set='new_train')
    tfidf_vector = TfidfVectorizer(tokenizer=tokenizer, lowercase=False, sublinear_tf=True)
    dt_matrix_train = tfidf_vector.fit_transform(comments_train)
    # dt_matrix_train = normalize(dt_matrix_train, norm='l1', axis=0)

    comments_test = get_comments(set='new_test')
    dt_matrix_test = tfidf_vector.transform(comments_test)
    # dt_matrix_test = normalize(dt_matrix_test, norm='l1', axis=0)

    tf_idf_classify(set='new_train', folder=folder + "/tf_idf_classifier/")
    train_n_test(tf_idf_classifiers, tf_idf_voting, dt_matrix_train, get_labels(set='new_train'), dt_matrix_test, get_labels(set='new_test'), folder=folder + "/tf_idf_classifier_tet/")

    # FEATURES
    lines_train = get_lines(serialized=False, serialize=True, set='new_train')

    feat_classify(set='new_train', folder=folder + "/feat_classifier/", lines=lines_train)

    features_train = get_features(set='new_train', scaled=True, lines=lines_train)
    lines_test = get_lines(serialized=False, serialize=True, set='new_test')
    features_test = get_features(set='new_test', scaled=True, lines=lines_test)

    train_n_test(feat_classifiers, feat_voting, features_train, get_labels(set='new_train'), features_test,
                 get_labels(set='new_test'), folder=folder + "/feat_classifier_tet/")

    # BOTH_CLASSIFIERS
    both_classify(set='new_train', folder=folder + "/both_classifier/", lines=lines_train)

    both_train = sparse.hstack((dt_matrix_train, features_train))
    both_test = sparse.hstack((dt_matrix_test, features_test))

    train_n_test(feat_classifiers, both_voting, both_train, get_labels(set='new_train'), both_test,
                 get_labels(set='new_test'), folder=folder + "/both_classifier_tet/")
